...
def prepare_lora_packet_json(data):
    ...
    print(f"   JSON bytes len: {len(json_bytes)} hex: {json_bytes.hex()[:200]}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        while True:
            ...
            # Prepare LoRa packet every LORA_PACKET_INTERVAL seconds
            loop_count += 1
            if (loop_count * SENSOR_READ_INTERVAL) % LORA_PACKET_INTERVAL == 0:
                # Choose JSON payload (application payload encrypted with AppSKey on-air)
                prepare_lora_packet_json(sensor_data)
                # Or keep binary compact format:
